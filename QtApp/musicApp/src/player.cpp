...
void Player::player_start_reply(QJsonObject &obj)
{
    ...
    if (result == "offline") {
        QMessageBox::warning(this, "Play", "Device is offline");
    }
    else if (result == "failure") {
        QMessageBox::warning(this, "Play", "Failed to start playback");
    }
...
void Player::closeEvent(QCloseEvent *event)
{
    ...
    e->accept();  // Accept the close event
}
...
void Player::on_playButton_clicked()
{
    ...
    else if (start_flag == 1 && suspend_flag == 1) {  // 暂停
        QJsonObject obj;
